...
function runPlaywright(
  ...
  return new Promise((resolve) => {
    ...
    const proc = spawn(PLAYWRIGHT_BIN, ["test", "--config", configFile], {
      cwd: PROJECT_ROOT,
      env: {
        ...cleanEnv,
...
describe("scenario: default config", () => {
  ...
  test("API calls arrive in order: build → test → finalize", () => {
    ...
    const idx = {
      ...
      finalize: paths.findIndex((p) =>
        p.startsWith("/api/build/status/calculate/"),
      ),
    };
  ...
  test("build request contains all required fields", () => {
    const body = server.firstRequestTo("/api/build")?.body as Record<
      string,
      unknown
    >;
  ...
  test("passing test payload is correct", () => {
    const t = getSubmittedTests(server).find((x) =>
      x.name.includes("passing test"),
    )!;
    expect(t.status).toBe("PASS");
    expect(t.is_rerun).toBe(false);
    expect(t.failure).toBeUndefined();
    expect(t.uid).toBe("passing test");
    expect(new Date(t.start_time).getTime()).toBeLessThanOrEqual(
  ...
  test("annotated test uses the ureport-uid annotation as uid", () => {
    const t = getSubmittedTests(server).find((x) =>
      x.name.includes("annotated uid test"),
    )!;
  ...
  test("tags from { tag: [...] } option appear in info.tags", () => {
    const t = getSubmittedTests(server).find((x) =>
      x.name.includes("tagged via option"),
    )!;
  ...
  test("non-ureport-uid annotations appear as extra keys in info", () => {
    const t = getSubmittedTests(server).find((x) =>
      x.name.includes("custom relation test"),
    )!;
  ...
  test("components annotations are collected into an array in info", () => {
    const t = getSubmittedTests(server).find((x) =>
      x.name.includes("components and teams test"),
    )!;
  ...
  test("every test payload includes duration in info as a human-readable string", () => {
    for (const t of getSubmittedTests(server)) {
      ...
      expect(t.info!.duration as string).toMatch(
        /^\d+ms$|^\d+\.\d+s$|^\d+m \d+s$|^\d+h \d+m$/,
      );
    }
  });
  ...
  test("test_relation calls arrive after build finalization", () => {
    const paths = server.requests.map((r) => r.path);
    const finalizeIdx = paths.findIndex((p) =>
      p.startsWith("/api/build/status/calculate/"),
    );
  ...
  test("relation for components+teams test has arrays for both fields", () => {
    const req = server
      .requestsTo("/api/test_relation")
      .find(
        (r) =>
          (r.body as Record<string, unknown>).uid ===
  ...
  test("custom annotations go into customs on the relation", () => {
    const req = server
      ...
      .find(
        (r) =>
          (r.body as Record<string, unknown>).uid === "custom relation test",
      )!;
    const customs = (req.body as Record<string, unknown>).customs as Record<
      string,
      unknown
    >;
...
describe("scenario: outputFile", () => {
  ...
  test("output file relations match unique uids from tests", () => {
    const content = JSON.parse(readFileSync(outputPath, "utf-8"));
    const relationUids: string[] = content.relations
      .map((r: { uid: string }) => r.uid)
      .sort();
    const uniqueTestUids = [
      ...new Set<string>(content.tests.map((t: UReportTestPayload) => t.uid)),
    ].sort();
...
describe("scenario: all optional parameters configured", () => {
  ...
  beforeAll(async () => {
    ...
    await server.start();
    await runPlaywright(
      path.join(FIXTURES, "full-params.config.ts"),
      server.port,
    );
  }, 90_000);
  ...
  test("test with explicit steps has a body array", () => {
    const t = getSubmittedTests(server).find((x) =>
      x.name.includes("test with explicit steps"),
    )!;
  ...
  test("failing step is marked as FAIL in body array", () => {
    const t = getSubmittedTests(server).find((x) =>
      x.name.includes("test with a failing step"),
    )!;
    const failStep = (t.body ?? []).find(
      (s: UReportStepPayload) => s.detail === "this step fails",
    );
  ...
  test("passing step is marked as PASS in body array", () => {
    ...
    const passStep = (t.body ?? []).find(
      (s: UReportStepPayload) => s.detail === "this step passes",
    );
  ...
  test("beforeEach/afterEach hooks generate setup/teardown entries", () => {
    ...
    expect((t.setup?.length ?? 0) + (t.teardown?.length ?? 0)).toBeGreaterThan(
      0,
    );
  });
  ...
  test("test with content attachments is submitted", () => {
    const attTest = getSubmittedTests(server).find(
      (x) => x.name === "test with content attachments",
    );
  ...
  test("JSON step has correct attachment content and content-type", () => {
    ...
    const jsonStep = attTest!.body?.find(
      (s: UReportStepPayload) => s.detail === "JSON step",
    );
  ...
  test("curl step has correct attachment content and content-type", () => {
    ...
    const curlStep = attTest!.body?.find(
      (s: UReportStepPayload) => s.detail === "curl step",
    );
  ...
  test("step without attach call has no attachment field", () => {
    const plainTest = getSubmittedTests(server).find((x) =>
      x.name.includes("test with explicit steps"),
    );
    const plainStep = plainTest?.body?.find(
      (s: UReportStepPayload) => s.detail === "prepare data",
    );
...
describe("scenario: saveRelations disabled", () => {
  ...
  beforeAll(async () => {
    ...
    await server.start();
    await runPlaywright(
      path.join(FIXTURES, "no-relations.config.ts"),
      server.port,
    );
  }, 90_000);
...
describe("scenario: batch submission (batchSize: 2)", () => {
  ...
  test("first three batches contain 2 tests each", () => {
    for (const i of [0, 1, 2]) {
      const batch = server.requestsTo("/api/test/multi")[i]?.body as {
  ...
  test("last batch contains 1 test", () => {
    const batch = server.requestsTo("/api/test/multi")[3]?.body as {
      tests: UReportTestPayload[];
    };
  ...
  test("finalize is called once, after all batches", () => {
    const paths = server.requests.map((r) => r.path);
    const testIdxs = paths.reduce<number[]>(
      (acc, p, i) => (p === "/api/test/multi" ? [...acc, i] : acc),
      [],
    );
    const finalizeIdx = paths.findIndex((p) =>
      p.startsWith("/api/build/status/calculate/"),
...
describe("scenario: retries", () => {
  ...
  test("first attempt of flaky test is FAIL, is_rerun=false", () => {
    ...
    expect(first.failure!.error_message).toContain(
...
describe("scenario: multi-project (chromium + firefox)", () => {
  ...
  beforeAll(async () => {
    ...
    await server.start();
    result = await runPlaywright(
      path.join(FIXTURES, "multi-project.config.ts"),
      server.port,
    );
  }, 90_000);
  ...
  test("the saved relation uid matches the test name", () => {
    const rel = server.firstRequestTo("/api/test_relation")?.body as Record<
      string,
      unknown
    >;

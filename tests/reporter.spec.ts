import { detectBrowser, detectDevice, detectEnvironments, detectSettings } from '../src/mapper';
...
describe('detectDevice', () => {
  ...
  test('Android Samsung UA + isMobile → MOBILE-SM-G965U', () => {
    expect(detectDevice(makeProject({
      isMobile: true,
      userAgent: 'Mozilla/5.0 (Linux; Android 8.0.0; SM-G965U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/67.0.3396.87 Mobile Safari/537.36',
...
describe('detectEnvironments', () => {
  test('baseURL on first project use → { baseURL }', () => {
    const config = makeConfig({ projects: [makeProject({ baseURL: 'https://staging.example.com' })] });
    expect(detectEnvironments(config)).toEqual({ baseURL: 'https://staging.example.com' });
  });
...
describe('detectSettings', () => {
  test('timeout, retries and workers all set → full object', () => {
    const config = makeConfig({ projects: [makeProject() as unknown as FullProject & { timeout: number; retries: number }], workers: 4 });
    ...
    expect(detectSettings(config)).toEqual({ timeout: 30000, retries: 2, workers: 4 });
  });
  ...
  test('only timeout set on first project → { timeout }', () => {
    const config = makeConfig({ projects: [{ timeout: 10000, retries: 0, use: {} } as unknown as FullProject] });
    expect(detectSettings(config)).toEqual({ timeout: 10000 });
  });

  test('only retries set on first project → { retries }', () => {
    const config = makeConfig({ projects: [{ timeout: 0, retries: 3, use: {} } as unknown as FullProject] });
    expect(detectSettings(config)).toEqual({ retries: 3 });
  });

  test('all absent/zero → undefined', () => {
    const config = makeConfig({ projects: [{ timeout: 0, retries: 0, use: {} } as unknown as FullProject], workers: 0 });
